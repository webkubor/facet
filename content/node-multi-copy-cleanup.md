...
## 最贵的一课：我用新工具重复了旧工具的错误

上面那些都还只是操作层面的失误。这一条是**思路层面**的，而且我当时完全没意识到。

迁移时我把五个 LaunchAgent、`serve.sh`、`cs-doctor` 里的
...
```
~/.local/share/mise/installs/node/24.15.0/bin
```

改完很满意：nvm 引用 0 处，全部指向 mise。**但这两条路径犯的是同一个错——里面都带着版本号。**

`24.15.0` 写死在路径里，意味着「工具在哪」和「工具是哪个版本」被焊死在一起。node 升到 25 的那天，这七处会同时失效，而且照旧是延迟爆炸：下次那个服务重启时才炸，那时候谁还记得跟几个月前的迁移有关。
...
```
~/.local/share/mise/shims/node  →  /opt/homebrew/bin/mise
```

`shims/` 里每个命令都是指向 mise 本体的垫片，执行时由 mise 决定用哪个版本，**路径里没有版本号，永远不用改**。同类的还有 pyenv / rbenv / asdf 的 `shims/`、volta 的 `~/.volta/bin`、homebrew 的 `opt/<formula>/bin`（而不是 `Cellar/<version>/`）。

全部改成 shims 之后，那个为了救活旧会话而建的 `~/.nvm` 兼容软链也就可以删了——它本来就是个补丁。

> 这一条是被人当面点破的。我当时的原话是「根源是旧进程持有失效 PATH」，对方回了一句：
> **「为啥不从根源解决问题，你怎么那么喜欢打补丁」**。
>
> 回头看，我把**症状**当成了根源。旧进程拿不到 node 只是现象；真正的根源是「路径里写死版本号」这个设计，
> 它同时解释了：为什么 nvm 时代会出事、为什么迁移后还会出事、以及为什么升级 node 时会再出一次事。
> 抓住它，一次改七处，以后不再复发；抓不住，就只能一次次打补丁。

后来我把这条检查写进了工具里（见文末），因为**这类错误靠自觉是防不住的**——它在犯的当下看起来完全正确。
...
**一、这类问题为什么没有好工具？** —— 这条我动手写了一个。

现有的版本管理器都在回答「我装了什么」（`npm ls -g`、`brew list`、`pip list`），但真正咬人的是「我跑的是什么、它被谁遮蔽了、装的和跑的对得上吗」。这些信息完全可以自动算出来：扫 PATH、按 realpath 去重、跨位置对账版本号。

于是有了 [`path-guard`](https://github.com/webkubor/path-guard)（Rust，零第三方运行时依赖）：

```
$ path-guard which opencode
命令 opencode

▶ 实际执行  ~/.local/share/mise/shims/opencode
   运行时版本 1.18.25
   所属包     opencode-ai@1.18.25

✅ 没有其它副本，不存在遮蔽
```

事故当天同一条命令会显示 `运行时版本 1.17.8` 而 `所属包 opencode-ai@1.18.25` —— 一眼看出装的和跑的不是一个东西，不用挖三层。

写的过程里有三个判断值得记：

- **「多个路径」不等于「多份副本」**。homebrew 的 bin 布局大量用软链指向同一实体，只数路径个数会满屏误报，必须按 realpath 去重。
- **运行时版本和声明版本要分开取、且都要取**。两者分叉正是「更新没生效」的特征，只看任何一边都发现不了。
- **包名不是身份**。所以「所属包」那行连 name 一起显示——不然 `facet` 那种同名不同包的情况照样看不出来。

四个子命令里，只有 `pinned` 是**预防性**的：扫出 PATH 里写死版本号的路径段。它就是上面那节「最贵的一课」的产物 —— 因为那类错误在犯的当下看起来完全正确，靠自觉防不住，只能让工具去查。
...
现在的想法是把 `path-guard doctor` 挂进定期任务，但还没想清楚频率——太勤了是噪音，太疏了等于没有。也许更好的触发点不是时间，而是**事件**：每次换版本管理器、每次 `brew uninstall`、每次迁移之后跑一次。

**四、AI agent 帮你做这类迁移时，风险在哪。** 这次全程是我和 Claude 一起做的，它扫引用、改配置、跑验证都很快，但两次关键的判断失误都不是技术性的：一次是把症状当根源（旧进程拿不到 node → 去建兼容软链），一次是换了工具却照搬旧设计（nvm 的版本路径 → mise 的版本路径）。

两次都是我看着不对劲、直接说出来才纠正的。**它执行力很强，但「这个方向本身对不对」这个判断，还是得人来把。** 后一次我的原话是「为啥不从根源解决问题，你怎么那么喜欢打补丁」——效果立竿见影，一次改掉七处，还顺手把这条检查固化进了工具。
