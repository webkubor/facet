---
title: "webkubor"
subtitle: "资深前端工程师 / 跨境支付与 AI 平台前端负责人"
documentType: "resume"
date: "2026-08-04"
author: "Knowledge PDF Kit"
role: "Frontend Lead"
location: "Hangzhou"
contact: "hello@example.com / +86 138-0000-0000"
links: "github.com/webkubor / portfolio.example.com"
pageHeader: "Resume PDF Kit"
pageFooter: "Payment Platform / AI Platform / Frontend Infrastructure"
shareHeader: "一份按业务线组织的技术简历"
shareFooter: "把项目经历整理成更清楚、更可信、更好读的 PDF。"
---

# webkubor

长期负责跨境支付与 AI 平台方向的前端 0 到 1 建设，擅长把复杂业务拆成稳定的产品界面、可复用的前端架构和可交付的工程流程。经历覆盖支付中台、商户后台、代理商后台、多国家 H5 收银台、订单自动化工具链、AI 模型平台控制台、开发者文档站与前端基础设施。

## Tarspay 跨境支付业务线

### 支付中台 / 多角色后台

- 主导 Tarspay 管理后台、商户后台、代理商后台等多角色中台系统建设，覆盖账户、交易、结算、风控、订单、渠道和运营配置等高频业务模块。
- 通过 Vue 3、Vite、TypeScript、路由权限、状态管理、表单配置化和多语言能力，支撑不同角色在同一业务体系内协作。
- 把支付业务的复杂状态、异常处理、列表筛选、导出、详情追踪和操作反馈沉淀为稳定的后台交互模式。

### 业务价值

- 从 0 到 1 搭建核心管理端基础架构，并持续迭代多年。
- 降低新支付渠道、新商户配置和运营排查的前端接入成本。
- 让交易、结算、补单、风控等流程具备更清晰的可视化和可操作性。

## 多国家 H5 收银台站点群

### 国家与渠道覆盖

| 市场 | 代表站点 | 支付场景 |
| --- | --- | --- |
| 巴基斯坦 | PK Payment Web | JazzCash / Easypaisa |
| 孟加拉 | BD Payment Web / Reorder Money | bKash / Nagad / 补单 |
| 泰国 | TH Payment Web | PromptPay |
| 越南 | VN Payment Web | 二维码代收 |

### 关键职责

- 主导 H5 收银台基础项目和各国家独立站点建设，处理多语言、多币种、移动端适配、弱网容错、支付结果页和异常状态。
- 按国家支付习惯拆分 UI 流程，兼顾本地钱包操作路径、凭证回填、倒计时、订单查询和结果确认。
- 沉淀基础收银台能力，减少各国家站点重复搭建成本。

## 巴基斯坦站点群与订单自动化

### 商户站点与渠道支持

- 搭建巴基斯坦商户站点与渠道支持平台，服务本地商户展示、下单、订单查询和渠道配置。
- 配合业务侧快速验证不同商户、渠道和支付路径，缩短新场景落地周期。

### 订单自动化工具链

- 从 0 搭建订单自动化脚本、订单后台和 API 服务，用于聚合多商户、多账户订单拉取、状态同步和排查。
- 通过后台看板、Socket/API 服务和自动化任务，把人工盯单流程转成可监控、可回溯的系统能力。

## ModelGo AI 平台业务线

### 客户中心 / 控制台

- 主导 ModelGo 客户中心从 0 到 1 建设，覆盖鉴权、工作空间、API Key、模型体验、模型对比、用量、账单、日志、异步任务、Guardrails 等核心模块。
- 支持国内与海外两个市场的运行时配置、品牌主题、API 域名和文档入口切换。
- 持续优化模型体验页的上传、流式输出、对比视图、异常提示和移动/低视口适配。

### 官网与开发者文档站

- 搭建国内官网、海外官网独立 SSR 项目和开发者文档站，支撑官网展示、文档导航、Markdown 渲染、目录锚点、复制反馈和多市场配置。
- 将官网、控制台、文档站之间的品牌、入口和内容体验做统一收敛。

## 前端基础设施与工程化

### CLI / SDK / Design Tokens

- 从 0 搭建 ModelGo 配置 CLI，用于把 ModelGo 快速接入 Claude Code、Codex 等 AI 编程客户端。
- 搭建前端埋点 SDK，统一公共属性、批量上报、登录态鉴权和脱敏边界。
- 建设 design-tokens 单一真源，在控制台、官网、海外站、文档站之间复用品牌色、语义色、暗色主题和基础视觉变量。

### 工程习惯

- 熟悉 Vue 3、React、Vite、TypeScript、Astro SSR、Node/Express、Hono、Cloudflare Workers、Supabase、ECharts 等技术栈。
- 重视可维护性、可验证性和交付闭环，习惯用脚本、文档、检查命令和 page plan 把复杂任务变成可复用流程。

## 早期经历

- 2021.04 - 2022.03：智能合约、DApp、区块链钱包相关前端研发。
- 2020.05 - 2021.04：企业管理系统、前端工程和业务模块交付。
- 2018.06 - 2020.05：Node 微服务、微信小程序、H5 商城和后台管理系统。
