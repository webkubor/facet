...
## 加密文档：发布后必须做两件事（2026-09-16 立的规矩）

加密文档**不进站点的公开列表** —— 所以对别人来说「发布了」等于「没有」，「链接在哪」只能靠人给。发布后必须：

1. **跑自检：`pnpm verify:protected`**
   它做两件事：① 校验 `build-site.mjs` 里登记的每个受保护 slug 都在 `wrangler.toml` 的
   `PROTECTED_PATHS` 里（就是最容易漏的那一步）；② 逐个请求线上地址，确认返回 **401**（要密码）。
   实测过一次：漏配 `PROTECTED_PATHS` 会让付费手册**免密码公开**（返回 200）。
   现在 `deploy:protected` 已把自检串进流程（构建 → 部署 → **自检** → 推送），
   **自检不过就不会推卡片** —— 免得把公开链接当加密链接发出去。
2. **主动把链接报给开发者**：直接贴出「标题 + 完整 URL + 源码路径」，
   **不能只说「发布了」**；同时同步进 `docs/encrypted-delivery.md` 的清单（那里是唯一的链接台账）。

> 普通文章不受影响（它们能在站点列表里翻到，链接也可推断）；这条只针对加密文档。
